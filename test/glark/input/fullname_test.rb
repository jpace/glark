...
class Glark::FullNameTestCase < Glark::AppTestCase
  def test_with
    ...
    run_app_test expected, [ '-r', '--with-fullname', 'test/resources/.*e.txt$', 't.*e' ], dirname
  end
  ...
  def test_without
    ...
    run_app_test expected, [ '-r', '--without-fullname', 'test/resources/.*e.txt$', 't.*e' ], dirname
  end
end
