...
class Glark::BaseNameTestCase < Glark::AppTestCase
  def test_with
    ...
    run_app_test expected, [ '-r', '--with-basename', '^\w+e.txt$', 't.*e' ], dirname
  end
  ...
  def test_without
    ...
    run_app_test expected, [ '-r', '--without-basename', '^\w+e.txt$', 't.*e' ], dirname
  end
end
